...
const ProgramList: React.FC<ProgramListProps> = ({
  ...
  // Helper function to get department names from many-to-many relationship
  const getDepartmentNames = (program: ProgramSearchResult): string => {
    if (!program.departments || program.departments.length === 0) {
      return 'N/A';
    }
    const names = program.departments
      .map(pd => pd.department?.name)
      .filter(Boolean);
  ...
  // Get full department names for tooltip
  const getFullDepartmentNames = (program: ProgramSearchResult): string => {
    if (!program.departments || program.departments.length === 0) {
    ...
    <div className="bg-white rounded-lg shadow-sm overflow-hidden">
      <div className="border-b border-gray-200">
        <div className="grid grid-cols-12 px-6 py-3 text-sm font-medium text-gray-500">
          ...
          <div className="col-span-3">Department(s)</div>
            ...
            <div key={program.id} className="grid grid-cols-12 items-center px-6 py-4 hover:bg-gray-50">
              <div className="col-span-5">
                <div className="flex items-center">
                  <GraduationCap className="h-5 w-5 text-green-600 mr-3 flex-shrink-0" />
                  <div className="min-w-0">
                    <div className="font-medium text-gray-900 truncate">{program.programName}</div>
                    <div className="text-sm text-gray-500 truncate">
                      {program.degreeType || 'N/A'} · {program.university?.universityName || 'N/A'}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div 
                className="col-span-3 text-sm text-gray-900 truncate cursor-help"
                title={getFullDepartmentNames(program)}
              >
                {getDepartmentNames(program)}
              </div>
