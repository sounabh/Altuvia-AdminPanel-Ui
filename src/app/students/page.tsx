...
import { 
  ...
  MoreHorizontal, Bell, FileText, Clock,
  CheckCircle, AlertCircle, GraduationCap,
  MapPin, Award, Edit3, Eye,
  User, Loader2, Trash2,
  Plus, X
} from 'lucide-react';
...
// ==================== INITIAL STATE ====================
const initialFormData: CreateStudentFormData = {
  email: '',
  password: '',
  name: '',
  countries: [],
...
// ==================== CREATE STUDENT MODAL (SEPARATE COMPONENT) ====================
...
const CreateStudentModal = memo(function CreateStudentModal({ 
  isOpen, 
  onClose, 
  onSuccess 
}: CreateStudentModalProps) {
  const [formData, setFormData] = useState<CreateStudentFormData>(initialFormData);
  ...
  const handleInputChange = useCallback((field: keyof CreateStudentFormData, value: string) => {
  ...
  const addCountry = useCallback(() => {
    ...
    if (trimmed && !formData.countries.includes(trimmed)) {
      setFormData(prev => ({
        ...prev,
  ...
  }, [countryInput, formData.countries]);
  ...
  }, [courseInput, formData.courses]);

  const removeCourse = useCallback((course: string) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.countries.length > 0) {
        submissionData.countries = formData.countries;
      }

      if (formData.courses.length > 0) {
        submissionData.courses = formData.courses;
      }

      if (formData.studyLevel) {
        submissionData.studyLevel = formData.studyLevel;
      }

      if (formData.gpa) {
        submissionData.gpa = formData.gpa;
      }

      if (formData.testScores) {
        submissionData.testScores = formData.testScores;
      }

      if (formData.workExperience) {
        submissionData.workExperience = formData.workExperience;
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-6" autoComplete="off">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                name="student-new-secret"
                id="student-new-secret"
                autoComplete="off"
                autoCorrect="off"
                autoCapitalize="off"
                spellCheck="false"
                ...
                className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition font-mono tracking-wider ${
                ...
                style={{ 
                  ...
                  // @ts-expect-error - CSS property for password masking
                  textSecurity: 'disc'
                }}
              />
              <p className="text-xs text-gray-500 mt-1">
                Password will be masked for security
              </p>
              {formErrors.password && (
                <p className="text-red-500 text-xs mt-1 flex items-center">
                  <AlertCircle className="w-3 h-3 mr-1" />
...
const ProfileInfoSection = memo(function ProfileInfoSection({ student }: { student: StudentDetailedInfo }) {
  return (
...
const SubscriptionInfoSection = memo(function SubscriptionInfoSection({ student }: { student: StudentDetailedInfo }) {
  return (
...
const SavedUniversitiesSection = memo(function SavedUniversitiesSection({ 
  universities 
}: { 
  universities: StudentDetailedInfo['savedUniversities'] 
}) {
  return (
...
const CVSection = memo(function CVSection({ cvs }: { cvs: StudentDetailedInfo['CV'] }) {
  return (
...
const CalendarEventsSection = memo(function CalendarEventsSection({ 
  events 
}: { 
  events: StudentDetailedInfo['calendarEvents'] 
}) {
  return (
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-xl shadow-2xl p-8 w-full max-w-6xl max-h-[90vh] overflow-y-auto my-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
          <PersonalInfoSection student={student} onToggleVerification={onToggleVerification} />
        ...
        <div className="mt-6 flex justify-end space-x-3 sticky bottom-0 bg-white pt-4 border-t-2 border-gray-200">
          ...
          <button 
            onClick={() => onCreateTask(student.id)}
...
const MetricsBar = memo(function MetricsBar({ metrics, isLoading }: MetricsBarProps) {
  if (isLoading) {
    return (
...
const StudentManagementSystem = () => {
  ...
  }, [selectedStudent?.id, fetchStudents, fetchStats]);

  const handleToggleEmailVerification = useCallback(async (id: string) => {
    setOpenMenuId(null);
  ...
  }, [selectedStudent?.id, fetchStudents, fetchStudentDetails]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-4 md:p-8">
      ...
      <MetricsBar metrics={metrics} isLoading={statsLoading} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {students.map(student => (
                <StudentCard 
                  key={student.id} 
                  student={student}
                  isMenuOpen={openMenuId === student.id}
                  onToggleMenu={(e) => toggleMenu(student.id, e)}
                  onViewDetails={() => fetchStudentDetails(student.id)}
                  onToggleVerification={() => handleToggleEmailVerification(student.id)}
                  onDelete={() => handleDeleteStudent(student.id)}
                  onSendEmail={() => sendEmail(student.id)}
                  onCreateTask={() => createTask(student.id)}
                />
              ))}
            </div>
          </>
      ...
      {showModal && selectedStudent && !modalLoading && (
        <StudentModal 
          ...
          onToggleVerification={handleToggleEmailVerification}
